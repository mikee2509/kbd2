...
(
  ID_PODOPIECZNEGO NUMBER(4) NOT NULL 
...
, CONSTRAINT PODOP_PK PRIMARY KEY 
  ...
  USING INDEX 
  (
      CREATE UNIQUE INDEX PODOP_PK_IDX ON PODOPIECZNI (ID_PODOPIECZNEGO ASC) 
  )
  ENABLE 
);
...
CREATE INDEX OP_BANK_PODOP_FK_IDX ON OPERACJE_BANKOWE (ID_PODOPIECZNEGO);

CREATE INDEX OP_BANK_SL_KL_FK_IDX ON OPERACJE_BANKOWE (ID_SLOWA_KL);

CREATE INDEX PODOP_KONT_FK_IDX ON PODOPIECZNI (ID_KONTA);

CREATE INDEX SL_KL_PODOP_FK_IDX ON SLOWA_KLUCZOWE (ID_PODOPIECZNEGO);
...
1) na podstawie slowa kluczowego - wtedy ID_SLOWA_KL wskazuje na uzyte slowo kluczowe a ID_PODOPIECZNEGO wskazuje na podopiecznego. 
2) w wyniku recznego przypisania - wtedy ID_SLOWA_KL ma wartosc NULL a ID_PODOPIECZNEGO wskazuje na podopiecznego. ';
...
COMMENT ON COLUMN OPERACJE_BANKOWE.ID_PODOPIECZNEGO IS 'Podopieczny do ktorego przypisana jest operacja.';
...
COMMENT ON COLUMN PODOPIECZNI.ID_PODOPIECZNEGO IS 'Klucz sztuczny.';
...
COMMENT ON COLUMN SLOWA_KLUCZOWE.ID_PODOPIECZNEGO IS 'Podopieczny do ktorego nalezy dane slowo kluczowe.';
...
BEGIN
    SELECT ID_PODOP_SEQ.NEXTVAL INTO :NEW.ID_PODOPIECZNEGO FROM SYS.DUAL;
END;
/
