...
BEGIN
    ...
    FROM 
        (
            ...
            GROUP BY K.ID_KONTA ORDER BY COUNT(P.ID_PODOPIECZNEGO) ASC
        )
    WHERE ROWNUM = 1;    
END;
/
...
BEFORE INSERT
    ON OPERACJE_BANKOWE
FOR EACH ROW 
WHEN (NEW.ID_PODOPIECZNEGO IS NULL)
BEGIN
    BEGIN
        SELECT S.ID_PODOPIECZNEGO, MIN(S.ID_SLOWA_KL) INTO :NEW.ID_PODOPIECZNEGO, :NEW.ID_SLOWA_KL        
        --FROM SLOWA_KLUCZOWE WHERE REGEXP_LIKE (:NEW.TYTUL, WARTOSC);
        FROM SLOWA_KLUCZOWE S        
        JOIN PODOPIECZNI P ON S.ID_PODOPIECZNEGO = P.ID_PODOPIECZNEGO
        WHERE P.ID_KONTA = :NEW.ID_KONTA_ODBIORCY AND INSTR(UPPER(:NEW.TYTUL), WARTOSC) > 0
        GROUP BY S.ID_PODOPIECZNEGO;
    EXCEPTION
...
CREATE OR REPLACE TRIGGER fkntm_slowa_kl_podop_trg BEFORE
    UPDATE OF ID_PODOPIECZNEGO ON slowa_kluczowe FOR EACH ROW
    WHEN (new.ID_PODOPIECZNEGO <> old.ID_PODOPIECZNEGO)
BEGIN
